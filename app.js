...
function handleDialogFlowAction(sender, action, messages, contexts, parameters) {
    switch (action) {
        ...
        setTimeout(function(){
            ...
            {
                type:"web_url",
                url:"https://tim-pylon-23469.herokuapp.com/",
                title:"Track order"
            },
            {
                type:"phone_number",
        ...
        case "detailed-application":
            if (fbService.isDefined(contexts[0]) &&
                (contexts[0].name.includes('job') || contexts[0].name.includes('job-application-details_dialog_context'))
                ...
                let previous_job = (fbService.isDefined(contexts[0].parameters.fields['last-job'])
                    && contexts[0].parameters.fields['last-job'] != '') ? contexts[0].parameters.fields['last-job'].stringValue : '';
                let years_of_experience = (fbService.isDefined(contexts[0].parameters.fields['years-of-exp'])
                    && contexts[0].parameters.fields['years-of-exp'] != '') ? contexts[0].parameters.fields['years-of-exp'].stringValue : '';
                ...
                if (phone_number != '' && user_name != '' && previous_job != '' && years_of_experience != ''
                    && job_vacancy != '') {
                    f (phone_number == '' && user_name != '' && previous_job != '' && years_of_experience == '') 
...
async function greetUserText(userId) {
    ...
    if (user) {
        fbService.sendTextMessage(userId, "Welcome " + user.first_name + '! ' +
            'I\'m the BIAS Bot.I can answer frequently asked questions for you ' +
...
function receivedPostback(event) {
    ...
    switch (payload) {
        ...
        break;
        case 'JOB_APPLY':
        //get feedbck with new jobs
        dialogflowService.sendEventToDialogFlow(sessionIds, handleDialogFlowResponse,senderID, 'job openings');
        break;
        case 'CHAT':
        //user wants to chat
        fbService.sendTextMessage(senderID,"I love chatting as well.Do you have any other questions for me? Feel free to ask");
        break;
        default:
...
function receivedAuthentication(event) {
    ...
    sendTextMessage(senderID, "Authentication successfull");
}
